...
<head>
    ...
    <style>
        ...
        .dataTables_wrapper .dataTables_filter {
            margin-bottom: 1rem;
            /* Espaço abaixo da caixa de pesquisa */
        }
        ...
        /* filtros inline */
        .filter-date {
            @apply w-32 px-2 py-1 border border-gray-300 rounded placeholder-gray-400;
        }

        .dataTables_filter input {
            @apply w-48 px-2 py-1 border border-gray-300 rounded placeholder-gray-400;
        }
            ...
            <div class="container mx-auto px-6 py-4 flex flex-wrap justify-between items-center">
                ...
                <div class="flex space-x-4 text-white font-medium items-center">
                    ...
                    <div class="flex items-center">
                        ...
                        Último: <span
                            class="font-medium ml-1">{{ !empty($experimentos) ? $experimentos[count($experimentos) - 1]['inicio'] : '-' }}</span>
                    </div>
        ...
        <div class="container mx-auto px-6 py-10">
            ...
            @if (!empty($experimentos))
                ...
                <div class="flex flex-wrap justify-between w-full mb-4">
                    ...
                    <!-- Formulário de importação à direita -->
                    <form action="{{ route('experimentos.importarCsv') }}" method="POST" enctype="multipart/form-data"
                        class="flex gap-2 items-center">
                        @csrf
                        <label for="arquivo" class="sr-only">Importar CSV</label>
                        <input type="file" name="arquivo" id="arquivo" accept=".csv" class="block text-sm text-gray-100 file:mr-4 file:py-2 file:px-4
                    file:rounded-full file:border-0
                    file:text-sm file:font-semibold
                    file:bg-[#26557d] file:text-white
                    hover:file:bg-[#1f4566] transition" required>

                        <button type="submit"
                            class="flex items-center px-4 py-2 bg-[#26557d] text-white rounded hover:bg-[#1f4566] transition shadow-lg">
                ...
                <div class="bg-gray-100 rounded-2xl shadow-lg p-6 space-y-4">
                    ...
                    <div class="overflow-x-auto -mx-6 px-6">
                        <table id="tabelaExperimentos" class="min-w-full divide-y divide-gray-200 table-auto ">
                            ...
                            <tbody
                                class="bg-white divide-y divide-gray-200 odd:bg-white even:bg-gray-50 hover:shadow-sm transition">
                                @foreach ($experimentos as $experimento)
                                    ...
                                    <tr class="hover:bg-indigo-100/50 transition-colors duration-150">
                                        ...
                                        <td
                                            class="{{ $maxTemp > 30 ? 'text-red-600 font-medium' : 'text-green-600 font-medium' }}">
                                            {{ $maxTemp }}
                                        </td>
                                        <td>{{ $avgTemp }}</td>
                                        <td class="text-right actions space-x-2">
                                            ...
                                            @if(Auth::user()->isAdmin())
                                                <form action="{{ route('experimentos.destroy', $experimento['id']) }}" method="POST"
                                                    onsubmit="return confirm('Confirma?');" class="inline">
                                                    @csrf @method('DELETE')
